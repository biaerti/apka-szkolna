...
// hook nic nie robi.
//
// Do tego kanal realtime (subscribeTodayRecapEvents): zmiana w recap_events na
// drugim urzadzeniu (plus z telefonu, uwaga z widoku "Sala") wywoluje pull od
// razu. Polling zostaje jako zabezpieczenie, gdyby websocket padl.

import { useEffect } from 'react';
import { isSupabaseConfigured } from '../supabase';
import { pullTodayRecapEvents, subscribeTodayRecapEvents } from './sync';
...
export function useTodayEventsPull(intervalMs: number = INTERVAL_MS): void {
  useEffect(() => {
    ...
    const unsubscribe = subscribeTodayRecapEvents();
