...
const TOPICS: Topic[] = [
  {
    ...
    notebookNote: [
      'ŚWIAT OPOWIEŚCI', '',
      'Świat przedstawiony to wszystko, co autor umieścił w historii:',
      '• czas - kiedy dzieją się wydarzenia,',
      '• miejsce - gdzie się dzieją,',
      '• bohaterowie - kto bierze w nich udział,',
      '• wydarzenia - co się dzieje,',
      '• narrator - kto opowiada.', '',
      'Autor tworzy tekst, a narrator opowiada historię. Narrator może mówić jako bohater („wszedłem”) albo obserwator („Antek wszedł”).', '',
      'Przykład: „Wieczorem Lena zbudowała bazę w lesie”. Czas: wieczór. Miejsce: las. Bohaterka: Lena. Wydarzenie: budowa bazy.',
    ].join('\n'),
    questions: [
  ...
  },
  {
    ...
    notebookNote: [
      ...
      'Podmiot liryczny nie musi być poetą. Poeta jest autorem, a osoba mówiąca należy do świata wiersza.', '',
      'W prozie historię opowiada narrator. W wierszu mówi podmiot liryczny.', '',
      'Szukaj wskazówek: „ja”, „czuję”, „myślę”, „marzę”, „jesteśmy”.',
      '„Być sobą” oznacza znać swoje mocne strony i żyć w zgodzie ze swoimi wartościami, a nie grać kogoś dla cudzej aprobaty.',
    ].join('\n'),
    questions: [
    ...
    makeSlides: (previousSetId) => [
      ...
      slideTask('Z1', 'Przeczytaj:\n\n„W lobby stoję całkiem cicho,\nchoć w słuchawkach dudni bit.\nNie mam skina jak bohater,\nale mam swój własny styl.”\n\nZapisz: kto może być podmiotem lirycznym? Wypisz **dwie wskazówki** z tekstu. Czy wiemy, kto jest autorem? Uzasadnij.', 5 * 60, 'narrator'),
      slideTask('Z2', 'Porównaj dwa teksty:\n\n**A.** „Nina zamknęła grę. Była dumna z wyniku.”\n\n**B.** „Zamykam grę. Jestem dumna z wyniku.”\n\nZapisz, kto mówi w każdym tekście: **narrator-obserwator**, **narrator-bohater** czy **podmiot liryczny**. Uwaga: jeden opis nie wystarcza - ważne jest także to, czy tekst jest prozą, czy wierszem.', 5 * 60, 'wiersz'),
      slideTask('Z3', 'Napisz czterowersowy tekst pod tytułem **„Ja online i ja offline”**.\n\n- mów w pierwszej osobie,\n- pokaż jedną rzecz, którą lubisz, i jedną, która jest dla ciebie ważna,\n- użyj jednego rzeczownika, czasownika i przymiotnika - podkreśl je i podpisz: **R, CZ, P**.', 8 * 60, 'wiersz'),
  ...
  },
  {
    ...
    textbookPage: 18,
    notebookNote: [
      'NOTATKA, KTÓRA POMAGA', '',
      'Dobra notatka wybiera informacje najważniejsze - nie kopiuje wszystkiego.', '',
      '• Punkty - gdy wyliczamy fakty albo kolejne kroki.',
      '• Tabela - gdy porównujemy co najmniej dwie rzeczy.',
      '• Mapa myśli - gdy pokazujemy skojarzenia i związki.',
      '• Sketchnotka - gdy prosty symbol pomaga zapamiętać pojęcie.', '',
      'Zasada 3K: KRÓTKO, KONKRETNIE, CZYTELNIE.',
      'Najpierw czytam cały fragment. Potem wybieram słowa-klucze. Na końcu układam je w najlepszej formie.',
    ].join('\n'),
    questions: [
    ...
    makeSlides: (previousSetId) => [
      ...
      slideTask('Z1', 'Oto chaotyczna wiadomość z klasowego czatu:\n\n„Ej pamiętajcie że jutro polski na drugiej lekcji, trzeba mieć podręcznik i zeszyt, czytamy strony 22-24, a grupa Oli przynosi też flamastry, prezentacje pokazujemy w piątek.”\n\nZamień ją w czytelną notatkę **punkt po punkcie**. Nadaj jej krótki tytuł i zostaw tylko informacje potrzebne uczniowi.', 6 * 60, 'wiadomosc'),
      slideTask('Z2', 'Dwaj twórcy pokazali poradniki do budowania domu w Minecraft:\n\n**Film A:** 45 sekund, 3 kroki, same napisy, łatwy.\n**Film B:** 6 minut, 8 kroków, komentarz głosowy, trudniejszy.\n\nZrób tabelę porównującą oba filmy. Sam wybierz **cztery nagłówki wierszy**. Pod tabelą napisz, który poradnik wybierasz i dlaczego.', 7 * 60),
      slideTask('Z3', 'Zrób małą mapę myśli pod hasłem **„Dobra opowieść”**.\n\nWykorzystaj wiedzę z pierwszej lekcji: czas, miejsce, bohaterowie, wydarzenia, narrator. Do każdej gałęzi dopisz po jednym własnym przykładzie. Jedną informację zastąp prostym symbolem lub rysunkiem.', 7 * 60, 'swiatPrzedstawiony'),
  ...
  },
  {
    ...
    notebookNote: [
      ...
      'Głoski słyszymy i wymawiamy. Litery widzimy i piszemy.',
      'Jedną głoskę czasem zapisujemy dwiema literami: sz, cz, rz, ch, dz, dż, dź.', '',
      'Samogłoski: a, ą, e, ę, i, o, u (ó), y. Bez samogłoski nie ma sylaby.',
      'Liczba sylab zwykle odpowiada liczbie słyszanych samogłosek.', '',
      'Przykłady:',
      '• chmura - 6 liter, 5 głosek, 2 sylaby: chmu-ra,',
      '• szafa - 5 liter, 4 głoski, 2 sylaby: sza-fa,',
      '• dżem - 4 litery, 3 głoski, 1 sylaba.', '',
      'Wyrazy przenosimy do następnej linii tylko między sylabami.',
    ].join('\n'),
    questions: [
  ...
  },
  {
    ...
    notebookNote: [
      'MALOWANIE SŁOWEM', '',
      'Epitet to określenie rzeczownika. Najczęściej jest przymiotnikiem.',
      ...
      'Nie każde określenie jest ciekawe. Zamiast „fajny skin” napisz dokładniej: „neonowy skin z pękniętą maską”.', '',
      'Autoportret może być obrazem, zdjęciem albo opisem siebie. Opis słowny pokazuje nie tylko wygląd, ale też charakter i zainteresowania.',
    ].join('\n'),
    questions: [
    ...
    makeSlides: (previousSetId) => [
      ...
      slideTask('Z1', 'Dwa opisy tej samej sceny:\n\n**A.** „Awatar wszedł do zamku. Zobaczył korytarz i drzwi.”\n\n**B.** „Samotny awatar wszedł do opuszczonego zamku. Zobaczył mroczny korytarz i żelazne drzwi.”\n\nWypisz epitety z tekstu B. Następnie jednym zdaniem wyjaśnij, jak zmieniły obraz lub nastrój sceny.', 5 * 60, 'opis'),
      slideTask('Z2', 'Ulepsz opis, ale nie używaj słów **fajny, super, ładny**:\n\n„W Robloxie zobaczyłem parkour. Były tam platformy, światła i tunel.”\n\nPrzepisz tekst, dodając co najmniej **5 trafnych epitetów**. Podkreśl epitety, a rzeczowniki, które określają, otocz kółkiem.', 7 * 60, 'opis'),
      slideTask('Z3', 'Zaprojektuj słowny kadr komiksu zatytułowany **„Sekundę przed katastrofą”**.\n\nNapisz 3-4 zdania. Podaj czas, miejsce, bohatera i wydarzenie. Użyj minimum **4 epitetów**, które budują napięcie. Na końcu dopisz, kto jest narratorem.', 8 * 60, 'swiatPrzedstawiony'),
...
function slideTask(code: string, body: string, timerSec: number, art?: SlideArt): Slide { return { id: newId(), kind: 'task', code, body, timerSec, art, zeszyt: true }; }
