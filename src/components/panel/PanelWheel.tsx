// Rozwiniety plywajacy panel: naglowek (uchwyt + wybor klasy + zwin/zamknij),
// kolo, przycisk Krec, ramka z nazwiskiem i dwa przyciski oceny.
//
// Uklad jest siostra TaskWheelDrawer (szuflada w prezentacji) - te same zasady
// kola na lekcji, ta sama kolejnosc elementow - ale scisniety do okna ~360 px
// i na zaokraglonym, ciemnym tle, bo panel lezy NA cudzym oknie i musi byc od
// niego wyraznie odcinany.
...
export interface PanelWheelProps {
  ...
  /** Lista uwag - stan trzyma Panel, bo Esc i klawisze kola musza o niej wiedziec. */
    ...
    <div className="flex h-full w-full flex-col overflow-hidden rounded-2xl bg-gray-900 text-gray-200 shadow-2xl ring-1 ring-gray-700">
      ...
      >
        ...
        <div className="flex min-w-0 flex-1 gap-1 overflow-x-auto">
          {classes.map((c) => (
            <button
              key={c.id}
              type="button"
              onClick={() => onClassId(c.id)}
              className={
                c.id === classId
                  ? 'shrink-0 rounded-md bg-accent-600 px-2 py-1 text-xs font-semibold text-white'
                  : 'shrink-0 rounded-md px-2 py-1 text-xs text-gray-400 hover:bg-gray-800'
              }
            >
              {c.name}
            </button>
        ...
        <button
          ...
          className="shrink-0 rounded-md px-2 py-1 text-gray-400 hover:bg-gray-800 hover:text-gray-200"
        >
          −
        </button>
        {onZamknij && (
      ...
      <div className="relative flex min-h-0 flex-1 flex-col">
        ...
        <div className="flex shrink-0 flex-col items-center gap-1 px-3">
          {pulaPusta ? (
            <>
              <p className="text-center text-xs text-gray-400">Wszyscy obecni już dziś odpowiadali</p>
              {!wheel.allowRepeats && (
                <button
                  type="button"
        ...
        {uwagiOtwarte && (
          <PanelUwagi classId={classId} students={wheel.classStudents} onZamknij={() => onUwagi(false)} />
        )}
      </div>
    </div>
  );
