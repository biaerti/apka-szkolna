...
export function Bohaterowie({ className }: { className?: string }) {
  return (
    ...
    >
      ...
      <text x={130} y={186} textAnchor="middle" fontSize={21} fontWeight={800} fill={C.plus}>
        GŁÓWNY
      </text>
      <text x={130} y={212} textAnchor="middle" fontSize={17} fill={C.white}>
        przez cały czas
      </text>

      <Postac x={350} y={78} r={19} kolor={C.pas} />
      <text x={350} y={186} textAnchor="middle" fontSize={21} fontWeight={800} fill={C.pas}>
        DRUGOPLANOWY
      </text>
      <text x={350} y={212} textAnchor="middle" fontSize={17} fill={C.white}>
        tylko czasem
      </text>

      {/* Pasek scen: w kazdej jest glowny, drugoplanowy tylko w dwoch */}
      {SCENY.map(([nr, zDrugoplanowym], i) => (
        <g key={nr} transform={`translate(${34 + i * 110} 238)`}>
